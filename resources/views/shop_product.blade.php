@if($products->count() > 0)
      ...
      <div class="rounded position-relative fruite-item">
        <div class="fruite-img">
          <img src="{{ asset('uploads/products/' . $product->image) }}" class="img-fluid w-100 rounded-top" alt=""
            style="height: 214px; width:306px;">
        </div>
  ...
  @endforeach
@else
  <div class="col-md-6 col-lg-6 col-xl-4"></div>
  <img src="{{ asset('img/no product found.jpg') }}" alt="">
  </div>
@endif
...
@php
  ...
  $projectsOnPage = array_slice($products->toArray(), $offset, $perpage);
 @endphp
<div class="col-12">
  ...
  <div class="pagination d-flex justify-content-center mt-5">
    ...
    @for ($i = 1; $i <= ceil(count($products) / $perpage); $i++)
      <a href="?page={{ $i }}" class="rounded {{ $i == $currentPage ? 'active' : '' }}">
        {{ $i }}
      </a>
    @endfor
